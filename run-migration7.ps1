# DigiCard - Phase 2 / Step 2b: element variants, free colours, poems and a real event date.
# Run from the Package Manager Console:   .\run-migration7.ps1
# Log: migration7-log.txt
#
...
if (-not $PSScriptRoot) {
    Write-Host "Run this as a file (.\run-migration7.ps1)." -ForegroundColor Yellow
    return
}
Set-Location -LiteralPath $PSScriptRoot

$log = Join-Path $PSScriptRoot 'migration7-log.txt'
...
$m1 = Invoke-Step "create migration CardVariants (templates)" {
    dotnet ef migrations add CardVariants --context TemplatesDbContext --project ".\src\Modules\DigiCard.Modules.Templates" --startup-project ".\src\DigiCard.DbMigrator" --output-dir "Infrastructure\Migrations"
}
if ($m1 -ne 0) { Stop-Here "TEMPLATES MIGRATION NOT CREATED - stopping. The database was not touched."; return }

# ------------------------------------------------------------------ 3. invitations migration
$m2 = Invoke-Step "create migration DraftEventDate (invitations)" {
    dotnet ef migrations add DraftEventDate --context InvitationsDbContext --project ".\src\Modules\DigiCard.Modules.Invitations" --startup-project ".\src\DigiCard.DbMigrator" --output-dir "Infrastructure\Migrations"
}
...
Invoke-Step "variants, poems and the event date" {
    ...
    try {
        ...
        # Every template's layout must have been rewritten with the new style fields.
        $cmd = $conn.CreateCommand()
        $cmd.CommandText = @"
SELECT COUNT(*) AS Total,
       SUM(CASE WHEN Elements LIKE '%"variant"%' THEN 1 ELSE 0 END) AS WithVariant
FROM templates.Templates
"@
        $r = $cmd.ExecuteReader()
        while ($r.Read()) { "templates: {0}   carrying the new style fields: {1}   (expected 11 / 11)" -f $r['Total'], $r['WithVariant'] }
        $r.Close()
        ""

        $cmd2 = $conn.CreateCommand()
        $cmd2.CommandText = "SELECT Poet, LEFT(REPLACE(Text, CHAR(10), ' / '), 60) AS Line FROM templates.Poems ORDER BY SortOrder"
        $r2 = $cmd2.ExecuteReader()
        $n = 0
        while ($r2.Read()) { $n++; "{0,-8} {1}" -f $r2['Poet'], $r2['Line'] }
        $r2.Close()
        ""
        "poems: $n   (expected 7)"
        ""
...
WHERE object_id = OBJECT_ID('invitations.Drafts') AND name IN ('EventDate','EventTime')
"@
        $r3 = $cmd3.ExecuteReader()
        while ($r3.Read()) { "invitations.Drafts.{0} : {1}" -f $r3['name'], $r3['Type'] }
        $r3.Close()
    }
