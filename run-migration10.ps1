# DigiCard - drafts can be reopened and saved over: address and last-changed columns.
#
#   .\run-migration10.ps1
#
...
if (-not $PSScriptRoot) {
    Write-Host "Run this as a file (.\run-migration10.ps1)." -ForegroundColor Yellow
    return
}
Set-Location -LiteralPath $PSScriptRoot

$log = Join-Path $PSScriptRoot 'migration10-log.txt'
...
$created = Invoke-Step "create migration DraftUpdatedAt (invitations)" {
    dotnet ef migrations add DraftUpdatedAt --context InvitationsDbContext --project ".\src\Modules\DigiCard.Modules.Invitations" --startup-project ".\src\DigiCard.DbMigrator" --output-dir "Infrastructure\Migrations"
}
...
WHERE object_id = OBJECT_ID('invitations.Drafts')
  AND name IN ('Elements','Address','EventDate','EventTime','EventEndTime','UpdatedAt')
ORDER BY name
"@
        $r = $cmd.ExecuteReader()
        $n = 0
...
Write-Log "Next: F5, then /studio - save a card, and it should appear under /my-cards."
